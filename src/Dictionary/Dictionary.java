...
public class Dictionary {
    ...
    public static void main(String[] args) {
        ...
        words.getWords().add(new Word("Hello", "xin chao"));
        words.getWords().add(new Word("Bear", "con gau"));
        words.getWords().add(new Word("Apple", "qua tao"));
        words.getWords().add(new Word("cat", "con meo"));

        int n = words.getWords().size();
        Dictionary.sortWord(words, 0, n - 1);
        for (int i = 0; i < n; i++) {
            System.out.println(words.getWords().get(i).getWord_target()+" "+words.getWords().get(i).getWord_explain());
        }
        String wordSearch = "HeLLo";
        System.out.println(Dictionary.binarySearch(words, 0, n - 1, wordSearch));
    }
}
